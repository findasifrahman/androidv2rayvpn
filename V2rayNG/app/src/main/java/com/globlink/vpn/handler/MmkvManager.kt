...
import com.globlink.vpn.AppConfig.PREF_IS_BOOTED
import com.globlink.vpn.AppConfig.PREF_ROUTING_RULESET
import com.globlink.vpn.dto.AssetUrlItem
import com.globlink.vpn.dto.ProfileItem
import com.globlink.vpn.dto.RulesetItem
import com.globlink.vpn.dto.ServerAffiliationInfo
...
//        profileStorage.encode(key, JsonUtil.toJson(profile))
    ...
    fun decodeSubscriptions(): List<Pair<String, SubscriptionItem>> {
        Log.d(AppConfig.ANG_PACKAGE, "Inside decodesubs")
        initSubsList()
    ...
    fun decodeSubsList(): MutableList<String> {
        ...
        Log.d(AppConfig.ANG_PACKAGE, "inside decode sub list," + json.toString())
        return if (json.isNullOrBlank()) {
            mutableListOf()
        } else {
            JsonUtil.fromJson(json, Array<String>::class.java).toMutableList()
