...
import com.globlink.vpn.dto.Language
...
object Utils {
    ...
    fun decode(text: String?): String {
        Log.d(ANG_PACKAGE, "Attempting to decode text, first 20 chars: ${text?.take(20)}...")
        val result = tryDecodeBase64(text) ?: text?.trimEnd('=')?.let { 
            Log.d(ANG_PACKAGE, "First decode failed, trying without padding")
            tryDecodeBase64(it) 
        }.orEmpty()
        Log.d(ANG_PACKAGE, "Decode result length: ${result.length}")
        return result
    }
    ...
    fun tryDecodeBase64(text: String?): String? {
        try {
            Log.d(ANG_PACKAGE, "Trying standard base64 decode")
            val decoded = Base64.decode(text, Base64.NO_WRAP).toString(Charsets.UTF_8)
            Log.d(ANG_PACKAGE, "Standard base64 decode successful, length: ${decoded.length}")
            Log.d(ANG_PACKAGE, "Decoded  is: ${decoded}")
            return decoded
        } catch (e: Exception) {
            Log.i(ANG_PACKAGE, "Parse base64 standard failed: ${e.message}")
        }
        try {
            Log.d(ANG_PACKAGE, "Trying URL-safe base64 decode")
            val decoded = Base64.decode(text, Base64.NO_WRAP.or(Base64.URL_SAFE)).toString(Charsets.UTF_8)
            Log.d(ANG_PACKAGE, "URL-safe base64 decode successful, length: ${decoded.length}")
            return decoded
        } catch (e: Exception) {
            Log.i(ANG_PACKAGE, "Parse base64 url safe failed: ${e.message}")
        }
        return null
    }
    ...
    fun isXray(): Boolean = (ANG_PACKAGE.startsWith("com.globlink.vpn"))
